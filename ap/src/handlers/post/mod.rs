...
pub async fn get_all_posts_by_user(db: Data<Db>, user_id: Path<Id>) -> impl Responder {
    match Post::get_all_by_user(&db.pool, user_id.into_inner()).await {
        Ok(p) => respond::ok(p),
        Err(e) => respond::err(e)
    }
}
pub async fn new_post_in_user(user_id: Path<Id>) -> impl Responder {
...
pub async fn new_post_in_topic(db: Data<Db>, topic_id: Path<Id>, post: Json<Post>) -> impl Responder {
    match post.into_inner().insert(&db.pool).await {
        Ok(p) => {
...
pub async fn new_post_in_group(db: Data<Db>, group_id: Path<Id>, post: Json<Post>) -> impl Responder {
    match post.into_inner().insert(&db.pool).await {
        Ok(p) => {
...
pub async fn get_all_posts_in_record(db: Data<Db>, topic_id: Path<Id>, post: Json<Post>) -> impl Responder {
    "".to_string()
}
pub async fn get_all_posts_in_group(db: Data<Db>, group_id: Path<Id>) -> impl Responder {
...
pub async fn get_by_id(db: Data<Db>, post_id: Path<Id>) -> impl Responder {
...
pub async fn delete_by_id(db: Data<Db>, post_id: Path<Id>) -> impl Responder {
